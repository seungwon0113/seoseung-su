...
from favorites.forms.favorite_toggle import FavoriteToggleForm
...
class FavoriteToggleView(LoginRequiredMixin, View):
    def post(self, request: HttpRequest) -> HttpResponse:
        form = FavoriteToggleForm(request.POST)
        user = cast(User, request.user)
        if form.is_valid():
...
class FavoriteCreateView(LoginRequiredMixin, View):
    def post(self, request: HttpRequest) -> HttpResponse:
        ...
        if form.is_valid():
            product_id = form.cleaned_data['product_id']
            result = FavoriteService.add_to_favorites(user, product_id)
            ...
            request.session['favorite_error'] = "잘못된 요청입니다."
